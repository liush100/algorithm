/*
    Student Name: Shuhao Liu. Assignment2 transfer2.c
    gcc transfer1.c -pthread -o transfer2

    gcc -Wall -pedantic-errors transfer2.c -o transfer2 -lpthread
*/
...
int main(int argc, char *argv[]){/*main thread is a fill thread*/ 
    ...
    if(2 == *(int *)retvalLocation){
        sem_destroy(&spaces); /* destroy semaphore */
        sem_destroy(&items);
        free(buffer);
        exit(0);
    }else{
...
void fill_function (void *ptr ){
    ...
    while(flag ==1){     
        if (sem_wait(&spaces) < 0){/* down semaphore */
        ...
        /* END CRITICAL REGION */        
        if (sem_post(&items) < 0){/* up semaphore */
...
void *drain_function(void *ptr){
    ...
    while(flag == 1){
        if (sem_wait(&items) < 0){/* down semaphore */
        ...
        if (sem_post(&spaces) < 0){/* up semaphore */
